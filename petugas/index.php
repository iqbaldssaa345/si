...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['action_quick_presensi'])) {
    ...
    $ket = trim($_POST['keterangan'] ?? 'Quick Entry Dashboard');
    ...
    if ($destId > 0 && $jml > 0) {
        ...
        if ($ins->execute([$userId, $destId, $today, $hari, $jml, $jenis, $ket])) {
            setFlash('success', 'Presensi cepat <strong>' . number_format($jml) . ' wisatawan</strong> berhasil dicatat!');
        } else {
            setFlash('danger', 'Gagal mencatat presensi cepat.');
        }
    } else {
        setFlash('danger', 'Mohon pilih destinasi dan masukkan jumlah pengunjung yang valid.');
    }
...
$stmtValidasi = $pdo->query("SELECT COUNT(*) FROM pemesanan WHERE status_kunjungan = 'sudah_digunakan' AND DATE(waktu_checkin) = CURDATE()")->fetchColumn();
$stmtValidasiOrang = $pdo->query("SELECT COALESCE(SUM(jumlah_tiket), 0) FROM pemesanan WHERE status_kunjungan = 'sudah_digunakan' AND DATE(waktu_checkin) = CURDATE()")->fetchColumn();

// Total Seluruh Kunjungan yang Dicatat Petugas Ini
...
// Kunjungan Terbaru yang Diinput Petugas Ini
...
// 5 Tiket Terakhir yang Check-in Masuk Hari Ini
...
<main class="admin-main">
    ...
    <div>
      <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
        <span class="badge-luxury badge-luxury-warning">
          <i class="fa-solid fa-id-badge"></i> Petugas Lapangan & Loket Gate
        </span>
        <span style="font-size: 0.8rem; color: #64748b;">
          <?= formatTanggalIndo($today) ?> • <span id="petugasClock" style="font-weight: 700; color: #0d9488;"><?= date('H:i:s') ?> WIB</span>
        </span>
      </div>
      ...
      <p style="font-size: 0.85rem; color: #64748b; margin: 0.2rem 0 0 0;">
        Selamat bertugas, <strong><?= htmlspecialchars($_SESSION['user_nama']) ?></strong>! Gate operasional siap memvalidasi tiket dan mencatat kehadiran wisatawan.
      </p>
    </div>

    <div style="display: flex; align-items: center; gap: 0.6rem; flex-wrap: wrap;">
      <a href="<?= BASE_URL ?>petugas/validasi_tiket.php" class="btn btn-primary btn-sm btn-luxury-pulse" style="font-weight: 800; box-shadow: 0 4px 14px rgba(13, 148, 136, 0.4); padding: 0.6rem 1.25rem;">
        <i class="fa-solid fa-qrcode"></i> Scan E-Tiket Gate
      </a>
      <a href="<?= BASE_URL ?>petugas/input_kunjungan.php" class="btn btn-secondary btn-sm" style="background: #ffffff; border: 1px solid #cbd5e1; font-weight: 700; padding: 0.6rem 1rem;">
  ...
  <div class="grid grid-cols-4 gap-5 mb-7">
    ...
    <div class="kpi-card-luxury kpi-primary" style="padding: 1.25rem;">
      <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.75rem;">
        ...
        <span class="badge-luxury badge-luxury-primary" style="font-size: 0.7rem;">Shift Anda</span>
      </div>
      <span style="font-size: 0.72rem; font-weight: 800; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; display: block;">
        Input Kunjungan Hari Ini
      </span>
      <h2 style="font-size: 1.55rem; font-weight: 900; color: #0f172a; margin: 0.2rem 0 0 0; font-family: 'Outfit', sans-serif;">
    ...
    <a href="<?= BASE_URL ?>petugas/validasi_tiket.php" style="text-decoration: none; color: inherit;">
      <div class="kpi-card-luxury kpi-success" style="cursor: pointer; padding: 1.25rem; border: 1px solid #e2e8f0;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.75rem;">
          <div class="kpi-icon-wrap" style="width: 44px; height: 44px; font-size: 1.15rem; background: #dcfce7; color: #059669;">
            <i class="fa-solid fa-door-open"></i>
          </div>
          <span class="badge-luxury badge-luxury-success" style="font-size: 0.7rem;">Gate Masuk</span>
        </div>
        <span style="font-size: 0.72rem; font-weight: 800; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; display: block;">
        ...
        <span style="font-size: 0.72rem; color: #059669; font-weight: 700; display: block; margin-top: 0.35rem;">
    ...
    <a href="<?= BASE_URL ?>petugas/riwayat_input.php" style="text-decoration: none; color: inherit;">
      <div class="kpi-card-luxury kpi-info" style="cursor: pointer; padding: 1.25rem; border: 1px solid #e2e8f0;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.75rem;">
          ...
          <span class="badge-luxury badge-luxury-info" style="font-size: 0.7rem;">Total Kontribusi</span>
        </div>
        <span style="font-size: 0.72rem; font-weight: 800; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; display: block;">
          Semua Wisatawan Tercatat
        </span>
        <h2 style="font-size: 1.55rem; font-weight: 900; color: #0f172a; margin: 0.2rem 0 0 0; font-family: 'Outfit', sans-serif;">
    ...
    <div class="kpi-card-luxury kpi-warning" style="padding: 1.25rem; border: 1px solid #e2e8f0;">
      <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.75rem;">
        <div class="kpi-icon-wrap" style="width: 44px; height: 44px; font-size: 1.15rem; background: #fef3c7; color: #d97706;">
          <i class="fa-solid fa-mountain-sun"></i>
        </div>
        <span class="badge-luxury badge-luxury-warning" style="font-size: 0.7rem;">Siap Melayani</span>
      </div>
      <span style="font-size: 0.72rem; font-weight: 800; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; display: block;">
      ...
      <span style="font-size: 0.72rem; color: #d97706; font-weight: 700; display: block; margin-top: 0.35rem;">
        <i class="fa-solid fa-circle-check"></i> Status operasional normal
      </span>
    </div>
  ...
  <div class="grid grid-cols-12 gap-6 mb-7">
    ...
    <!-- Left Column: Quick Presensi Entry Widget (7 cols) -->
    <div class="col-span-7" style="grid-column: span 7 / span 7;">
      <div class="card p-6 bg-white shadow-sm" style="border-radius: 1.25rem; border: 1.5px solid #ccfbf1; height: 100%;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.25rem; padding-bottom: 0.75rem; border-bottom: 1px solid #f1f5f9;">
          <div>
            <span class="badge-luxury badge-luxury-primary" style="font-size: 0.7rem;">
              <i class="fa-solid fa-bolt text-amber"></i> Fast Entry
            </span>
            <h3 style="font-size: 1.15rem; font-weight: 800; color: #0f172a; margin: 0.2rem 0 0 0;">
              Catat Presensi Wisatawan Cepat
            </h3>
          </div>
          <span style="font-size: 0.75rem; color: #64748b;">Langsung Simpan</span>
        </div>
        ...
        <form action="<?= BASE_URL ?>petugas/index.php" method="POST">
          ...
          <div class="form-group mb-4">
            <label class="form-label font-bold text-xs uppercase text-muted">Pilih Destinasi Wisata <span class="text-danger">*</span></label>
            <select name="destinasi_id" class="form-control" style="font-weight: 700; border-radius: 0.65rem;" required>
              <option value="">-- Pilih Lokasi Objek Wisata --</option>
              <?php foreach ($destinasiList as $d): ?>
                <option value="<?= $d['id'] ?>"><?= htmlspecialchars($d['nama_destinasi']) ?> (<?= htmlspecialchars($d['lokasi']) ?>)</option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div class="form-group">
              <label class="form-label font-bold text-xs uppercase text-muted">Jumlah Wisatawan <span class="text-danger">*</span></label>
              <div style="display: flex; align-items: center; gap: 0.35rem;">
                <button type="button" class="btn btn-secondary btn-sm" onclick="adjustQuickCount(-1)" style="padding: 0.4rem 0.75rem; font-weight: 800;">-</button>
                <input type="number" name="jumlah_pengunjung" id="quickJmlInput" class="form-control text-center" value="1" min="1" style="font-weight: 800; font-size: 1.1rem; font-family: 'Outfit', sans-serif;" required>
                <button type="button" class="btn btn-secondary btn-sm" onclick="adjustQuickCount(1)" style="padding: 0.4rem 0.75rem; font-weight: 800;">+</button>
              </div>
              <!-- Quick Add Buttons -->
              <div style="display: flex; gap: 0.25rem; margin-top: 0.4rem;">
                <button type="button" class="btn btn-light btn-xs" onclick="setQuickCount(5)" style="font-size: 0.72rem; padding: 0.15rem 0.45rem; border: 1px solid #e2e8f0;">+5</button>
                <button type="button" class="btn btn-light btn-xs" onclick="setQuickCount(10)" style="font-size: 0.72rem; padding: 0.15rem 0.45rem; border: 1px solid #e2e8f0;">+10</button>
                <button type="button" class="btn btn-light btn-xs" onclick="setQuickCount(20)" style="font-size: 0.72rem; padding: 0.15rem 0.45rem; border: 1px solid #e2e8f0;">+20</button>
                <button type="button" class="btn btn-light btn-xs" onclick="setQuickCount(50)" style="font-size: 0.72rem; padding: 0.15rem 0.45rem; border: 1px solid #e2e8f0;">+50</button>
              </div>
            </div>

            <div class="form-group">
              <label class="form-label font-bold text-xs uppercase text-muted">Jenis Kunjungan</label>
              <select name="jenis_kunjungan" id="quickJenisInput" class="form-control" style="border-radius: 0.65rem; font-weight: 600;">
          ...
          <div class="form-group mb-5">
            <label class="form-label font-bold text-xs uppercase text-muted">Catatan Loket (Opsional)</label>
            <input type="text" name="keterangan" class="form-control text-sm" placeholder="Cth: Rombongan Keluarga, Pembelian Loket Pintu Barat..." style="border-radius: 0.65rem;">
          </div>

          <button type="submit" class="btn btn-primary btn-block btn-lg" style="font-weight: 800; border-radius: 0.75rem; padding: 0.85rem; box-shadow: 0 4px 14px rgba(13, 148, 136, 0.35);">
    ...
    <div class="col-span-5" style="grid-column: span 5 / span 5;">
      <div class="card p-6 bg-white shadow-sm" style="border-radius: 1.25rem; border: 1.5px solid #e2e8f0; height: 100%; display: flex; flex-direction: column;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem; padding-bottom: 0.75rem; border-bottom: 1px solid #f1f5f9;">
          <h3 style="font-size: 1.05rem; font-weight: 800; color: #0f172a; margin: 0; display: flex; align-items: center; gap: 0.4rem;">
            <i class="fa-solid fa-qrcode text-primary"></i> Live Gate Check-in
          </h3>
          <a href="<?= BASE_URL ?>petugas/validasi_tiket.php" class="text-xs font-bold text-primary" style="text-decoration: none;">
          ...
          <?php else: ?>
            <ul style="list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.5rem;">
              <?php foreach ($gateLiveList as $gl): ?>
                <li style="display: flex; align-items: center; justify-content: space-between; padding: 0.65rem 0.85rem; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 0.75rem;">
                  <div style="overflow: hidden; flex: 1; padding-right: 0.5rem;">
                    <div style="display: flex; align-items: center; gap: 0.4rem;">
                      <code style="font-size: 0.78rem; font-weight: 800; color: #0f766e; background: #ccfbf1; padding: 0.1rem 0.35rem; border-radius: 4px;">
                        <?= htmlspecialchars($gl['kode_booking']) ?>
                      </code>
                      <span style="font-size: 0.75rem; font-weight: 700; color: #1e293b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                        <?= htmlspecialchars($gl['nama_pemesan'] ?? $gl['nama_user']) ?>
                      </span>
                    </div>
                    <span style="font-size: 0.72rem; color: #64748b; display: block; margin-top: 0.15rem;">
                      <?= htmlspecialchars($gl['nama_destinasi']) ?> • <strong style="color: #0d9488;"><?= $gl['jumlah_tiket'] ?> Org</strong>
                    </span>
                  </div>
                  <div style="text-align: right;">
                    <span style="font-size: 0.72rem; font-weight: 800; color: #059669; background: #dcfce7; padding: 0.2rem 0.5rem; border-radius: 9999px; display: inline-block;">
        ...
        <div style="margin-top: 1rem; padding-top: 0.75rem; border-top: 1px solid #f1f5f9; text-align: center;">
          <a href="<?= BASE_URL ?>petugas/validasi_tiket.php" class="btn btn-light btn-sm btn-block" style="font-size: 0.78rem; font-weight: 700; color: #0d9488; border: 1px solid #ccfbf1;">
            <i class="fa-solid fa-camera"></i> Mulai Scan Kamera Sekarang
          </a>
        </div>
      </div>
    </div>
  ...
  <div class="card-table-luxury" style="width: 100%;">
    <div style="padding: 1.25rem 1.5rem; border-bottom: 1px solid #e2e8f0; display: flex; align-items: center; justify-content: space-between; background: #ffffff;">
      <div>
        <h3 style="font-size: 1.05rem; font-weight: 800; color: #0f172a; margin: 0;">
          Catatan Input Presensi Loket Terakhir Anda
        </h3>
        <p style="font-size: 0.78rem; color: #64748b; margin: 0.15rem 0 0 0;">
          Daftar entri wisatawan yang baru saja Anda inputkan ke sistem.
        </p>
      </div>

      <a href="<?= BASE_URL ?>petugas/riwayat_input.php" class="btn btn-secondary btn-sm" style="font-size: 0.78rem; border-radius: 0.5rem;">
          ...
          <?php if (empty($kunjunganTerbaru)): ?>
            <tr>
              <td colspan="6" style="text-align: center; padding: 2.5rem 1rem; color: #94a3b8;">
                <p style="font-size: 0.9rem; font-weight: 600; color: #64748b; margin: 0;">Belum ada catatan kunjungan yang Anda simpan hari ini.</p>
              </td>
            </tr>
          <?php else: ?>
            <?php foreach ($kunjunganTerbaru as $kt): ?>
              <tr>
                ...
                <td>
                  <strong class="text-dark" style="font-size: 0.92rem;"><?= htmlspecialchars($kt['nama_destinasi']) ?></strong>
                  <span class="text-xs text-muted" style="display: block; margin-top: 0.1rem;">
                    <i class="fa-solid fa-location-dot" style="color: #ef4444; font-size: 0.7rem;"></i> <?= htmlspecialchars($kt['lokasi']) ?>
                  </span>
                </td>
                <td>
                  <strong style="color: #1e293b; font-size: 0.88rem;"><?= formatTanggalIndo($kt['tanggal_kunjungan']) ?></strong>
                  <span class="badge badge-light text-xs" style="margin-left: 0.3rem;"><?= htmlspecialchars($kt['hari']) ?></span>
                </td>
                <td style="text-align: center;">
                  <div style="display: inline-block; background: #ccfbf1; border: 1px solid #99f6e4; padding: 0.3rem 0.75rem; border-radius: 9999px;">
...
function autoCheckJenis(count) {
  ...
  if (count >= 10 && jenisSelect.value === 'individu') {
    jenisSelect.value = 'rombongan';
  } else if (count < 10 && jenisSelect.value === 'rombongan') {
